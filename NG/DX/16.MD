...
* Expand 𝑖 – na vrcholu zásobníku je neterminál 𝐴, který je přepsán na pravou stranu pravidla 𝑖, které má neterminál 𝐴 na levé straně
...
* Pokud lze 𝑁 přepsat na prázdný řetězec, pak First(𝑁) obsahuje i prázdný znak
  * Pro 𝑁 = _ε_ platí vždy First(_ε_) = { _ε_ }
...
* Cílem optimalizace je typicky jedno z:
  ...
  2. Menší nároky na operační paměť za běhu
...
* Pro zrychlení kódu za běhu (__i.__) vyndáme vše, co zdržuje:
  * Skoky – vyhodnocení podmínek
  * Vkládání funkcí – minimalizace skoků a ukládání kontextu → zrychlení
  * Cykly – u malých cyklů (pod 20 iterací) je výhodnější vložit kód přímo 𝑛-krát za sebe
  * Nepotřebné instrukce a duplicity – např. opakované přiřazení stejné hodnoty do stejné proměnné, opakované počítání stejného výrazu ve větším vzorci
  * Invariant cyklu – co nutně nemusí být v cyklu z něj vyjmeme ven
  * Předpočítat konstanty
  * Vektorizace operací
* Pro menší nároky na operační paměť za běhu (__ii.__):
...
* Pro malou paměťovou stopu (__iii.__):
  * Odstranit vše nepotřebné, všechny duplicity, výpočet všech dílčích výrazů
  * Malá paměťová stopa často implikuje dlouhé trvání kódu, protože nás nutí vynechat inlining, vkládání funkcí a skoků
* Pro malou spotřebu energie (__iv.__):
...
* Zavaděč má svůj binární formát, který podporuje; ve Windows např. .exe a .dll
* Druhy zavedení:
