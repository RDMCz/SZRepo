...
* Konečné automaty jsou ekvivalentní, pokud rozpoznávají stejný jazyk
  * Pro určení ekvivalence je třeba definovat ne/dosažitelnost každého stavu
  * Stav 𝑞 je dosažitelný, pokud existuje takové slovo 𝑤, že platí _δ_<sup>\*</sup>(𝑞₀, 𝑤) = 𝑞
* Stavy automatu dále dělíme na:
  * Užitečné (generující) – lze se z nich dostat do koncového stavu
  * Nadbytečné (negenerující) – nelze se z nich dostat do koncového stavu
* Automat s odstraněnými nedosažitelnými a nadbytečnými stavy je ekvivalentní tomu původnímu; nazýváme ho redukovaný KA

#### Totální (úplný) KA

* Deterministický konečný automat, ve kterém lze ve všech stavech reagovat na kterýkoliv symbol abecedy: ∀ 𝑞∈𝑄,𝑎∈Σ ∃ 𝑝∈𝑄 kde _δ_(𝑞,𝑎)=𝑝
* Ke každému DKA existuje ekvivalentní totální automat
* Chceme-li z neúplného automatu udělat totální, můžeme přidat jeden stav navíc, kam budou mířit „chybějící“ přechody, tento stav nebude koncový a budou se v něm cyklit všechny vstupní symboly

#### Jazyk stavu KA

* Změníme-li počáteční stav KA na nějaké 𝑞∈𝑄 a spočítáme jazyk přijímaný tímto automatem, získáme tzv. jazyk stavu 𝐿(𝑞)
* Dva různé stavy jsou ekvivalentní, pokud rozpoznávají stejný jazyk

#### Minimální KA

* KA je minimální, jestliže neexistuje žádný s ním ekvivalentní automat, který má menší počet stavů
* Postup minimalizace automatu:
  1. Odstranit nedosažitelné stavy
  2. Odstranit nadbytečné stavy (nyní máme redukovaný KA)
  3. Odstranit ekvivalentní stavy (nyní máme minimální KA)

#### Normovaný KA

* DKA bez nedosažitelných stavů, jehož stavy jsou označeny jednoznačně

#### Homogenní KA

* Pro každý stav platí, že všechny příchozí přechody jsou ohodnoceny stejným symbolem

<!--
### Operace s KA

* Automat sjednocený z 𝐿₁(𝐴₁) a 𝐿₂(𝐴₂) rozpoznává slova z jazyků 𝐿₁ anebo 𝐿₂
* Automat průniku 𝐿₁(𝐴₁) a 𝐿₂(𝐴₂) rozpoznává slova, která patří do jazyka 𝐿₁ a zároveň do jazyka 𝐿₂
* Doplněk KA rozpoznává Σ<sup>\*</sup> ∖ 𝐿
* Rozdíl vypočteme pomocí doplňku a průniku: 𝐴₁ - 𝐴₂ = 𝐴₁ ∩ -𝐴₂
* Zrcadlový obraz
* Zřetězení
* Iterace
-->
...
<!--
### Konečný překladový automat (KPA)

* Formální překlad je binární relace, která každému prvku z množiny formálního jazyka 𝐿₁ přiřazuje množinu jeho překladů z množiny formálního jazyka 𝐿₂
* Řetězce z 𝐿₁ jsou definiční obor a řetězce z 𝐿₂ jsou obor hodnot
* Pokud relace každému 𝑤 ∈ 𝐿₁ přiřazuje maximálně jeden prvek 𝑍(𝑤) ∈ 𝐿₂, pak se jedná o funkci s názvem _jednoznačný překlad_
* Překladová gramatika je nástroj pro generování řetězců z jazyka definičního oboru a k nim odpovídajícího výstupu z jazyka oboru hodnot
* Regulární překladová gramatika je rozpoznatelná KPA
* KPA = (𝑄, Σ, 𝐷, _δ_, 𝑞₀, 𝐹):
  * 𝑄 je množina stavů
  * Σ je množina vstupních symbolů
  * 𝐷 je konečná množina výstupních symbolů, 𝐷 ∩ Σ = ∅
  * _δ_ je přechodová funkce automatu
    * DKPA: 𝑄 × Σ → 𝑄 × 𝐷<sup>\*</sup>
    * NKPA: 𝑄 × Σ → 𝑃(𝑄 × 𝐷<sup>\*</sup>)
    * Na jeden přechod tedy KPA může generovat libovolně dlouhý konečný řetězec (včetně _ε_)
  * 𝑞₀ ∈ 𝑄 je počáteční stav
  * 𝐹 ⊆ 𝑄 je množina koncových stavů

#### Konfigurace KPA

* Konfigurace KPA je uspořádáná trojice (𝑞, 𝑥, ⓧ) ∈ 𝑄 × Σ<sup>\*</sup> × 𝐷<sup>\*</sup>
  * 𝑞 je aktuální stav automatu
  * 𝑥 je nepřečtená část vstupních symbolů
  * ⓧ je řetězec výstupních symbolů
* Počáteční konfigurace je (𝑞₀, 𝑤, _ε_), kde 𝑤 je celý řetězec vstupních symbolů
* Koncová konfigurace je (𝑞<sub>𝑓</sub>, _ε_, ⓧ<sub>𝑓</sub>)
  * 𝑞<sub>𝑓</sub> ∈ 𝐹
  * Vstupní řetězec se musí celý přečíst
  * ⓧ<sub>𝑓</sub> je finální řetězec výstupních symbolů
* Překlad 𝑢→𝑣 je tedy (𝑞₀, 𝑢, _ε_) →<sup>\*</sup> (𝑞<sub>𝑓</sub>, _ε_, 𝑣)
-->
